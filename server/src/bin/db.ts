...
export async function getFoodSearch(getFoodSearch: string) {
  const food = await query<RowDataPacket[]>(
    `SELECT * FROM mydb.food WHERE getFoodSearch`,
    { values: [getFoodSearch] }
  );
  if (food.length < 1) return null;
  else return food;
